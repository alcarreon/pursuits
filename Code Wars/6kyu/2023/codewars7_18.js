// Help Suzuki purchase his Tofu!

// Help Suzuki purchase his Tofu!

// Suzuki has sent a lay steward to market who will purchase some items not produced in the monastary gardens for the monks. The stewart has with him a large box full of change from donations earlier in the day mixed in with some personal items. You will be given a string of items representing the box.

// Sort through the items in the box finding the coins and putting aside anything else.

// You will be given a data structure similar to the one below.

// box = 'mon mon mon mon mon apple mon mon mon mon mon mon mon monme mon mon monme mon mon mon mon cloth monme mon mon mon mon mon mon mon mon cloth mon mon monme mon mon mon mon monme mon mon mon mon mon mon mon mon mon mon mon mon mon'

// Return the following in your solution.

// [count of mon coins in box, count of monme coins in box,sum of all coins value in box, minimum number of coins needed for Tofu]

// 100 ≤ cost ≤ 1000

// cost = 124

// returns

// [45, 5, 345, 6]
// The coins have the following values:

// monme = 60

// mon = 1

// Determine the minimum number of coins to pay for tofu. You must pay with exact change and if you do not have the correct change return “leaving the market”.

// If the cost of tofu is higher than your total amount of money also return “leaving the market”

// parameters integer and string
// return array or string
// examples
// pseudo code create an object to hold the count of mon and mone with an initial value of 0 for both, then split the string and filter out anything other than mon and monme, convert the object into an array and iterate over the array checking each element to see if it needs to be changed to match the value of the currency, reduce the array to get the total value of coins, create a counter variable and then use a for loop to see how many of the biggest dinominations can be used in the soultion check to see that there are not more coins being used than we have available

function buyTofu(cost, box) {
  let holder = { mon: 0, monme: 0 };
  let onlyCoins = box.split(" ").filter((x) => x == "mon" || x == "monme");
  onlyCoins.forEach((x) => (holder[x] = holder[x] + 1 || 1));
  let totalCoins = Object.entries(holder)
    .map((x) => {
      if (x[0] == "mon") {
        return ["mon", x[1] * 1];
      } else {
        return ["monme", x[1] * 60];
      }
    })
    .reduce((x, y) => x[1] + y[1]);
  let monmeCounter = 0;
  for (let i = 1; i <= holder["monme"]; i++) {
    if (i * 60 < cost) {
      monmeCounter++;
    }
  }
  let monCoins = cost - monmeCounter * 60;
  if (holder["monme"] < monmeCounter || holder["mon"] < monCoins) {
    return "leaving the market";
  } else {
    return [
      holder["mon"],
      holder["monme"],
      totalCoins,
      monmeCounter + monCoins,
    ];
  }
}

let box1 =
  "mon mon mon mon mon apple mon mon mon mon mon mon mon monme mon mon monme mon mon mon mon cloth monme mon mon mon mon mon mon mon mon cloth mon mon monme mon mon mon mon monme mon mon mon mon mon mon mon mon mon mon mon mon mon";

let box2 =
  "mon mon mon mon mon apple mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon monme mon mon mon mon cloth mon mon mon mon mon mon mon mon mon cloth mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon mon";

// console.log(buyTofu(124, box1)); // [45, 5, 345, 6]

// console.log(buyTofu(5, "mon monme")); // "leaving the market"

// console.log(buyTofu(674, "mon mon mon")); // "leaving the market"

// console.log(buyTofu(1, "monme mon mon monme")); // [2, 2, 122, 1]

// console.log(buyTofu(122, "monme mon mon monme")); // [2, 2, 122, 4]

// console.log(buyTofu(124, box2)); // [121, 1, 181, 65]
