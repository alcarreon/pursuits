...
const balance = (str) => {
  ...
  let splitArr = holder.split("\n").filter((x) => x != "");
  splitArr.forEach((x, y) => {
    if (y == 0) {
      let startNum = +x;
      result += `Original Balance: ${startNum.toFixed(2)}\r\n`;
      balance += +x;
    } else {
      let expense = x.lastIndexOf(" ");
      spending += +x.slice(expense);
      let someNum = +x.slice(expense);
      result += `${x.slice(0, expense)} `;
      result += someNum.toFixed(2);
      result += ` Balance ${(balance - spending).toFixed(2)}\r\n`;
    }
  });
  result += `Total expense  ${spending.toFixed(2)}\r\n`;
  result += `Average expense  ${(spending / (splitArr.length - 1)).toFixed(2)}`;
  return result;
};
console.log(balance(b1)); // `Original Balance: 1000.00\r
...
// console.log(balance(b2)); // `Original Balance: 1233.00\r
