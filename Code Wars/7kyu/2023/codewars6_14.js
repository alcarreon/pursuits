...
// return string
// examples
// pseudo code create a variable to hold an empty string, then split the string by each element, iterate over the array checking the index of the elements to see when the letters need to be swapped
...
function spoonerize(str) {
  ...
  splitStr.forEach((x, y) => {
    if (y == 0) {
      holder += splitStr[splitStr.indexOf(" ") + 1];
    } else if (y == str.indexOf(" ") + 1) {
      holder += splitStr[0];
    } else {
      holder += x;
    }
