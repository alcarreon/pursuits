...
// return boolean
// examples
// pseudo code create a variable to hold the first integer converted into a string, then create another variable for the larger of the two variable between the second and third agument, do the same with the smaller of the two numbers, finally slice the string using the smallest and biggest number, check to see if the digit is inside the sliced string

function checkDigit(num, ind1, ind2, dig) {
  const convertedNum = num.toString();
  const big = Math.max(ind1, ind2) + 1;
  const small = Math.min(ind1, ind2);
  const check = convertedNum.slice(small, big);
  return check.includes(dig.toString());
}
