...
// parameters object, numbers, string
// return number
// examples
// pseudo code find the alcohol consumed by taking the number of ounces times the alcohol content, then check to see what sex the user provided, use the formula to calculate the blood alcohol content then fix the number of decimal places to 4 finally convert the string into a number

function bloodAlcoholContent(drinks, weight, sex, time) {
  let content = drinks.ounces * drinks.abv;
  if (sex == "female") {
    return +(((content * 5.14) / weight) * 0.66 - 0.015 * time).toFixed(4);
  } else {
    return +(((content * 5.14) / weight) * 0.73 - 0.015 * time).toFixed(4);
  }
}
